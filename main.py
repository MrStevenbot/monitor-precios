import subprocess
import sys
subprocess.check_call([sys.executable, "-m", "pip", "install", "requests", "schedule", "beautifulsoup4", "lxml", "python-telegram-bot==13.15"])
...
def cmd_start(update: Update, context: CallbackContext):
    update.message.reply_text(
        ...
        "/umbral [número] — Cambiar % mínimo\n"
        "/pausa — Pausar alertas\n"
        "/activar — Reactivar alertas",
        parse_mode='HTML'
    )

def cmd_estado(update: Update, context: CallbackContext):
...
def cmd_buscar(update: Update, context: CallbackContext):
    if not context.args:
        update.message.reply_text("Uso: /buscar [producto]\nEjemplo: /buscar iphone")
        return
    query = ' '.join(context.args)
    update.message.reply_text(f"🔍 Buscando <b>{query}</b> ahora...", parse_mode='HTML')
    historial = cargar_historial()
    total = 0
    ...
    if total == 0:
        update.message.reply_text(f"Sin ofertas detectadas para <b>{query}</b> en este momento.", parse_mode='HTML')

def cmd_umbral(update: Update, context: CallbackContext):
    global UMBRAL_OFERTA
    if not context.args:
        update.message.reply_text(f"Umbral oferta actual: {UMBRAL_OFERTA}%\nUso: /umbral [número]\nEjemplo: /umbral 15")
        return
    try:
        nuevo = int(context.args[0])
        if nuevo < 5 or nuevo > 90:
            update.message.reply_text("El umbral debe estar entre 5 y 90.")
            return
        UMBRAL_OFERTA = nuevo
        update.message.reply_text(f"✅ Umbral de oferta actualizado: {UMBRAL_OFERTA}%\n🚨 Error de precio sigue en: {UMBRAL_ERROR}%")
    except:
        update.message.reply_text("Número inválido. Ejemplo: /umbral 15")

def cmd_error(update: Update, context: CallbackContext):
    global UMBRAL_ERROR
    if not context.args:
        update.message.reply_text(f"Umbral error de precio actual: {UMBRAL_ERROR}%\nUso: /error [número]\nEjemplo: /error 50")
        return
    try:
        nuevo = int(context.args[0])
        if nuevo < 5 or nuevo > 90:
            ...
            return
        UMBRAL_ERROR = nuevo
        update.message.reply_text(f"✅ Umbral de error de precio actualizado: {UMBRAL_ERROR}%\n🔥 Oferta sigue en: {UMBRAL_OFERTA}%")
    except:
        update.message.reply_text("Número inválido. Ejemplo: /error 50")

def cmd_pausa(update: Update, context: CallbackContext):
    global BOT_PAUSADO
    BOT_PAUSADO = True
    update.message.reply_text("⏸ Bot pausado. Las alertas automáticas están desactivadas.\nUsa /activar para reanudar.")

def cmd_activar(update: Update, context: CallbackContext):
    global BOT_PAUSADO
    BOT_PAUSADO = False
    update.message.reply_text("▶️ Bot reactivado. Las alertas automáticas están activas.")
# ─────────────────────────────────────────────────────────────
...
def iniciar_bot_comandos():
    ...
    dp.add_handler(CommandHandler("estado", cmd_estado))
    dp.add_handler(CommandHandler("buscar", cmd_buscar))
    dp.add_handler(CommandHandler("umbral", cmd_umbral))
    dp.add_handler(CommandHandler("error", cmd_error))
    dp.add_handler(CommandHandler("pausa", cmd_pausa))
    dp.add_handler(CommandHandler("activar", cmd_activar))
